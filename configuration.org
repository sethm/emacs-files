...
* Introduction
  ...
  *There is one more thing:* Emacs comes with a built-in version of Org
  ...
  The only other troublesome setup is to install the necessary
  fonts to make /Doom Mode Line/ look pretty. That can be done
  inside of a running emacs with the command:
...
** Email and News
   ...
   All of my email and news configuration is kept locally, and it
   varies by machine, so if the configuration file exists, I will load
   it here.

   #+BEGIN_SRC emacs-lisp
     (when (file-exists-p (expand-file-name "~/.emacs-mail.el"))
       (load (expand-file-name "~/.emacs-mail.el")))
   #+END_SRC
...
** Themes
   ...
   Note that after installing these, you should run
   =M-x all-the-icons-install-fonts=.
