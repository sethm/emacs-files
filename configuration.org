...
** Development Support
   ...
   Smartparens lets me save some keystrokes.
...
** Publishing
   ...
   After publishing a site, I always want to rsync it to my 
   web server. We can do that with a hook.

   #+BEGIN_SRC emacs-lisp
     (defun loomcom-rsync-www (in-file out-file)
       ...
       (let ((publish-dir loomcom-www-dir)
         ...
         (when (file-exists-p publish-dir)
           (shell-command
            (format
             "rsync -avqz --delete --delete-after %s %s"
             publish-dir
             remote-dir)))))

     (add-hook 'org-publish-after-publishing-hook 'loomcom-rsync-www)
   #+END_SRC
