...
#+BEGIN_SRC emacs-lisp
  (when window-system
    (let* ((dpr (/ (float (display-pixel-height)) ;; display ratio
                   (float (display-mm-height))))
           (height (if (> dpr 1.5) 140 100))
           (families '("Hack"
                       "Roboto Mono"
                       "Input Mono"
                       "Inconsolata"
...
#+BEGIN_SRC emacs-lisp
  (use-package doom-themes
    ...
    (set-face-foreground 'window-divider "#666666")
    (set-face-foreground 'window-divider-first-pixel "#666666")
    (set-face-foreground 'window-divider-last-pixel "#666666")
