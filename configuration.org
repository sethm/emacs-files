...
** Face Tweaks
   ...
   There are a few minor thing I like to tweak.

   First, I like my blocks to stand out.
   ...
   And next, I like to give my Org Mode faces a bit of a tweak.
     ...
     (if (file-exists-p "~/.org-agenda-setup.el")
       ...
       (progn
         (global-set-key (kbd "C-c o")
                         (lambda ()
                           (interactive)
                           (find-file "~/ownCloud/Notes/notes.org")))
         (setq org-agenda-files (file-expand-wildcards "~/ownCloud/Notes/*.org")
               org-archive-location (concat "~/ownCloud/Notes/Archive/%s::")
               org-default-notes-file "~/ownCloud/Notes/notes.org")))
   #+END_SRC
   ...
   #+BEGIN_SRC emacs-lisp
     (defun loomcom-rsync-www ()
       ...
       (let ((publish-dir loomcom-www-dir)
             (master-dir "neon.loomcom.com:/var/www/loomcom/")
	     (beta-dir "neon.loomcom.com:/var/www/beta/"))
